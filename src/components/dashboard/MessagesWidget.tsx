...
const initialMessages: Message[] = [
  {
    id: "1", from: "Sarah Chen", avatar: "SC",
    ...
    time: "10 min ago", read: false, project: "Brand Identity",
  },
  {
    id: "2", from: "Mike Torres", avatar: "MT",
    ...
    time: "45 min ago", read: false, project: "Website V2",
  },
  {
    id: "3", from: "Lisa Park", avatar: "LP",
  ...
  },
  {
    ...
    time: "5h ago", read: true, project: "Mobile App UI",
  },
  {
    id: "5", from: "Jordan Bell", avatar: "JB",
    ...
    <div className="flex gap-4 min-h-[400px]">
      <div className="w-[280px] shrink-0 space-y-3">
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search messages…" className="rounded-xl pl-9" />
        </div>
        <div className="space-y-1 max-h-[350px] overflow-y-auto">
          {filtered.map((msg) => (
            <button key={msg.id} onClick={() => selectMessage(msg)}
              className={cn("w-full text-left p-3 rounded-xl transition-colors", selected?.id === msg.id ? "bg-primary/10" : "hover:bg-secondary/50", !msg.read && "bg-secondary/30")}>
              <div className="flex items-center gap-2 mb-1">
                <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0", !msg.read ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground")}>{msg.avatar}</div>
              ...
              {msg.project && <span className="text-[10px] text-primary/70 pl-10">{msg.project}</span>}
            </button>
          ))}
        </div>
      </div>
        ...
        {selected ? (
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-sm font-bold">{selected.avatar}</div>
              <div>
              ...
              <div className="ml-auto">{selected.read ? <CheckCheck className="w-4 h-4 text-primary" /> : <Check className="w-4 h-4 text-muted-foreground" />}</div>
            </div>
            <div className="flex-1 p-4 rounded-xl bg-secondary/20 mb-4"><p className="text-sm leading-relaxed">{selected.fullMessage}</p></div>
            <div className="flex gap-2">
              <Input value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Type a reply…" className="rounded-xl flex-1" onKeyDown={(e) => e.key === "Enter" && reply.trim() && setReply("")} />
              <button onClick={() => reply.trim() && setReply("")} className="rounded-xl bg-primary text-primary-foreground p-2.5 hover:opacity-90 transition-opacity"><Send className="w-4 h-4" /></button>
            </div>
          </div>
        ) : (
          <div className="flex items-center justify-center h-full text-muted-foreground text-sm">Select a message to read</div>
        )}
      </div>
    </div>
  );
